...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Store user data in localStorage for all roles
      localStorage.setItem('user', JSON.stringify(userData));

      // Save inside context (for temporary state)
      login(userData);

      // Show toast notification (500ms) then redirect
      setShowToast(true);
      const redirectUrl = data.data?.redirect || (userData.role === 'admin' ? '/admin/admin-dashboard' : '/user-dashboard');
  ...
  // SUCCESS TOAST (500ms notification)
  if (showToast) {
    return (
      <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/30">
        <motion.div
          ...
          className="bg-gradient-to-r from-green-400 to-emerald-400 text-white px-8 py-4 rounded-lg shadow-2xl flex items-center gap-3"
        >
