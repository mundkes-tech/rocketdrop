...
export default function ProductDetails() {
  ...
  const isInWishlist = wishlistData?.items?.some((item) => item.id === params.id);
  ...
  const toggleWishlist = async () => {
    ...
    try {
      if (isInWishlist) {
        ...
        const res = await fetch(`/api/wishlists?product_id=${params.id}`, {
          method: 'DELETE',
          credentials: 'include',
        });
        ...
        const res = await fetch('/api/wishlists', {
          ...
          body: JSON.stringify({ product_id: parseInt(params.id) }),
        });
  ...
  const addToCart = async () => {
    ...
    try {
      // Add product with quantity to cart via context
      await addToCartContext({ ...product, quantity }, user);
  ...
  const handleBuyNow = async () => {
    ...
    try {
      await addToCartContext(product, user, quantity);
  ...
  return (
    ...
    >
          ...
          <header className="space-y-3">
             ...
             <div className="flex items-center space-x-4 pb-4 border-b border-gray-100">
                ...
                <span className="text-gray-600 text-sm font-medium">
                  {product.average_rating?.toFixed(1) || 'No'} ({product.review_count || 0} reviews)
                </span>
             </div>
          </header>
          ...
          {!isSupplier && (
            <div className="pt-4 space-y-4">
              ...
              <div className="flex gap-4">
                ...
                <Button 
                  variant="outline" 
                  className={`px-4 py-3 border-gray-300 hover:bg-gray-50 transition-all ${
                    isInWishlist ? 'bg-red-50 border-red-500' : ''
                  }`}
      ...
      {/* Reviews Section */}
      <ReviewSection productId={parseInt(params.id)} user={user} />
    </motion.div>
  );
}
